...
const translations = {
    ru: {
        ...
        videoCarouselTitle: '🎥 Видеообзоры гибридов кукурузы'
    },
    kz: {
        navLogo: '🌽 БахАгро',
        ...
        videoCarouselTitle: '🎥 Жүгері гибридтерінің бейнешолулары'
    }
};
...
function translatePage(lang) {
    ...
    document.getElementById('video-carousel-title').innerText = t.videoCarouselTitle;
...
const seedVideos = [
    { name: 'Афина', videoUrl: 'afinaV.mp4' },
    { name: 'Аврора', videoUrl: 'https://www.youtube.com/embed/dQw4w9WgXcQ' },
    { name: 'Атлант', videoUrl: 'https://www.youtube.com/embed/dQw4w9WgXcQ' },
    { name: 'Гоби', videoUrl: 'gobiV.mp4' },
    ...
    { name: 'new Афина + ФАО380', videoUrl: 'Afinafao380'}
];
...
function renderVideoCarousel() {
    ...
    track.innerHTML = '';
    seedVideos.forEach(item => {
        const card = document.createElement('div');
        card.className = 'video-card';
        card.innerHTML = `
            <!-- Added controlsList and disablePictureInPicture to free up space for the fullscreen button -->
            <video src="${item.videoUrl}" controls controlsList="nodownload noplaybackrate" disablePictureInPicture preload="metadata"></video>
            <div class="video-card-info">
                <h4>🌽 FENIKKS ${item.name}</h4>
            </div>
        `;
...
function moveCarousel(direction) {
    ...
    const cardWidth = cards[0].offsetWidth + 20; // Includes card width + gap
    
    // Updated breakpoints to match the new vertical card CSS
    let visibleCards = 4;
...
window.addEventListener('load', () => {
    ...
    renderVideoCarousel();
    document.getElementById('carousel-prev').addEventListener('click', () => moveCarousel('prev'));
    document.getElementById('carousel-next').addEventListener('click', () => moveCarousel('next'));
});
